...
class pandasBench(BaseDfBench):
    ...
    def one_hot_encoding(self, columns):
        ...
        self.df = pd.concat([self.df.drop(columns=columns), dummies], axis=1)
        return self.df
    ...
    def pivot(self, index, columns, values, aggfunc):
        ...
        """
        self.df = pd.pivot_table(self.df, index=index, values=values, columns=columns, aggfunc=aggfunc).reset_index()
        return self.df
    ...
    def unpivot(self, columns, var_name, val_name):
        ...
        """
        self.df = pd.melt(self.df, id_vars=list(set(list(self.df.columns.values)) - set(columns)), value_vars=columns, var_name=var_name, value_name=val_name)
        return self.df
